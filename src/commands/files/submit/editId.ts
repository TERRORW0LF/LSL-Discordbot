...
export async function run (interaction: CommandInteraction<"present">) {
    ...
    const patch = interaction.options.getString('patch', true),
    ...
    const runs = await getAllSubmits(interaction.guildId, { patch, season });
    ...
    try {
        await editSubmit(interaction.guildId, id, proof, { patch, season });
    } catch (error) {
        const embed: APIEmbed = {
